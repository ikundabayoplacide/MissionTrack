...
export class CompanyService {
  ...
  static async approveAndRejectCompany(companyId: string, data: ApproveRejectData) {
    ...
    // Check if company is already in the requested status
    if (company.status === data.status) {
      throw new Error(`Company is already ${data.status}`);
    }

    // Update database FIRST
    const newStatus = await company.update({ status: data.status, approveComment: data.comment });

    // Then send email notification (non-blocking - log errors but don't fail)
    try {
      await Mailer.notifyManagerAboutCompany(
        company.id,
        data.status === 'approved' ? "Company Approved ✅" : "Company Rejected ❌",
        `Your company "${company.companyName}" has been ${data.status}.<br/><br/><strong>Comment:</strong> ${data.comment}`
      );
    } catch (emailError) {
      console.error(`Failed to send email notification for company ${company.id}:`, emailError);
